...
export default async function handler(req, res) {
  ...
  // Si viene page_info, Shopify solo acepta eso + limit, nada más
  let params;
  if (rest.page_info) {
    params = new URLSearchParams({ page_info: rest.page_info, limit: rest.limit || 250 }).toString();
  } else {
    params = new URLSearchParams(rest).toString();
  }
  ...
  try {
    ...
    // Pasar Retry-After para que el frontend respete el rate limit (429)
